...
namespace cotsb
{
    namespace utils
    {
        std::random_device Utils::s_rd;
        std::mt19937 Utils::s_rand(Utils::s_rd());
        float Utils::s_time_since_start = 0.0f;

        const float Utils::degrees_to_radians = 0.01745329251994329576923690768489f;
        const float Utils::radians_to_degrees = 57.295779513082320876798154814105f;
        const float Utils::pi = 3.141592654f;

        const uint32_t Utils::max_uint = std::numeric_limits<uint32_t>::max();
        const int32_t Utils::max_int = std::numeric_limits<int32_t>::max();
        const int32_t Utils::min_int = std::numeric_limits<int32_t>::min();
